 ...
 * </ul>
 */
public class PatparAnnotatedTypeFactory extends AnnotatedTypeFactory {
    ...
    /** If we are inside a closure scope, then this will be non-null and
     * 	set to the innermost such closure scope.  Variables defined in this
     *  scope have normal type but other variables will have read-only type. */
    private Element innermostClosureScope;

    /**
     * Creates a new {@link PatparAnnotatedTypeFactory} that operates on a
     * particular AST.
     *
     * @param checker the checker to which this factory belongs
     * @param root the AST on which this type factory operates
     */
    public PatparAnnotatedTypeFactory(PatparChecker checker,
    ...
    public Element pushClosureScope(Element e) {
    	Element x = innermostClosureScope;
    	innermostClosureScope = e;
    	return x;
    }
    
    public void popClosureScope(Element cur, Element old) {
    	assert innermostClosureScope == cur;
    	innermostClosureScope = old;
    }
     ...
     *   
     *   <li> 7. Apply {@code @ReadOnly} to upvars from closure scopes.
     *
     * </ul>
     *
    ...
    protected void annotateImplicit(Tree tree, /*@Mutable*/ AnnotatedTypeMirror type) {
        ...
        // 7
        if (innermostClosureScope != null && elt != null) {
        	annotatePotentialClosureUpvar(tree, elt, type);
        }
    ...
    private void annotatePotentialClosureUpvar(Tree tree, Element elt,
    	...
    	case PARAMETER:
    	case FIELD:
    		for (Element e = elt; e != null; e = e.getEnclosingElement()) {
    			if (e == innermostClosureScope)
    				return;
    		}
    		break;
	...
	public AnnotatedDeclaredType getEnclosingType(TypeElement element, Tree tree) {
		AnnotatedDeclaredType dtype0 = super.getEnclosingType(element, tree);
